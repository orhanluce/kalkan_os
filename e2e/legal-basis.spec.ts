...
test("legal-basis guard: doğrulanmamış eşleme zorunlu kontrolü bloklar; doğrulanınca koşar", async ({ page }) => {
  ...
  // 3) Hukuk doğrulaması (service; rol kapısı K8 — bugün admin/uyum route işi
  //    değil, e2e'de service temsil eder): LEGAL_REVIEW → VERIFIED + atıf.
  ...
  for (const tablo of ["obligations", "obligation_control_mappings"] as const) {
    ...
    await db.from(tablo).update({ dogrulama_durumu: "LEGAL_REVIEW" }).eq("id", id);
  ...
  // 7) Temizlik: sentetik global zincir sökülür (fotoğraflar fixture cascade'i
