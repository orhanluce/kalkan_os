...
test("gizlilik: DSAR kanıt paketi mühürlenir + bağımsız offline doğrulanır", async ({ page }) => {
  ...
  try {
    ...
    await expect(page.getByText(/Kanıt paketi ✓/)).toBeVisible();

    // DB: paket + manifest hash + defter kaydı bağı.
    const { data: pkg } = await db
      .from("dsar_fulfillment_packages")
      .select("manifest_hash, ledger_entry_id, aciklanan_kategoriler")
      .eq("dsar_id", dsarId)
      .single();
