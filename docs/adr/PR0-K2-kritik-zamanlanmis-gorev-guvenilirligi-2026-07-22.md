...
**Tarih:** 22 Temmuz 2026 (güncelleme: aynı gün, §14 — dış tetik kararı KAPANDI)
**Durum:** REPO-İÇİ KISIM UYGULANDI — **K2_LOCAL_READY**; dış tetik kararı
VERİLDİ (§14: şimdilik C, ilk pilot öncesi A'ya geçiş, B reddedildi); canlı
migration + smoke doğrulaması hâlâ bekliyor — **K2_LIVE_VALIDATION_PENDING**
(§17). Bu turda production/staging Supabase'e bağlanılmadı, Supabase MCP
kullanılmadı, pg_cron canlıda değiştirilmedi, production migration
çalıştırılmadı, deploy yapılmadı, gerçek e-posta gönderilmedi, secret
okunmadı/yazılmadı, servis token oluşturulmadı, route açılmadı. K1 durumu
(ayrı, açık operasyonel borç) DEĞİŞMEDİ.
...
boşluğu KAPATMAZ ama ALTINDAKİ modeli (idempotency, claim, retry,
görünürlük) production-hazır hâle getirir — dış tetik HANGİSİ seçilirse
seçilsin AYNI güçlendirilmiş modele konuşacak. (Aynı gün eki: dış tetik
kararı §14'te KAPANDI — şimdilik C, pilot öncesi A.)
...
## 14. Kurucu kararı — dış tetik: ŞİMDİLİK C, pilot öncesi A'ya geçiş (22 Temmuz 2026, KAPANDI)

`ADR-dis-cron.md`'nin açık bıraktığı A/B/C kararı kurucu tarafından verildi:

**Şu aşamada Seçenek C (mevcut oto-drenaj + manuel drenaj) yürürlükte
kalır.** Yeni servis token'lı route AÇILMAZ, Edge Function KURULMAZ,
pg_cron+pg_net ile Hostinger route'una çağrı YAPILMAZ. `K2_LOCAL_READY`
durumu korunur; canlı otomatik tetik doğrulaması yapılmadığı için K2
tamamen KAPALI sayılmaz — durum **`K2_LIVE_VALIDATION_PENDING`**.

**İlk gerçek pilot başlamadan hemen önce veya pilot onboarding sırasında
Seçenek A'ya geçilecek** (pg_cron + pg_net → servis token'lı TEK TS drenaj
route'u). A'ya geçişin ZORUNLU kabul kriterleri (kurucunun listesi, aynen):
1. Ayrı servis token'lı route tasarımı.
2. Token'ın yalnız güvenli environment store'da saklanması.
3. Constant-time token doğrulama.
4. Rate-limit.
5. Audit log.
6. Payload minimizasyonu.
7. Route'un YALNIZ outbox drenajı yapması.
8. Kill-switch'in varsayılan ve etkili olması.
9. Duplicate/orphan/idempotency testleri.
10. Token rotasyon runbook'u.
11. Canlı smoke testi.
12. Backlog ve son başarılı çalışma görünürlüğü.

**Seçenek B bu aşamada REDDEDİLDİ:** yeni Edge Function altyapısı
gerektiriyor, ikinci motor riski taşıyor, A'ya karşı yeterli ek değer
sunmuyor.

Bu ADR'nin ürettiği altyapı (kill-switch, terminal hata, manuel retry,
sağlık özeti) C'de bugün, A'ya geçişte aynen çalışır — geçiş yalnız TETİK
mekanizmasını değiştirir, güvenilirlik katmanını değil.
...
1. Migration production'a uygulanır (`pnpm db:push`) — kurucunun açık
   onayını ister (bu ADR'nin KENDİSİ bu onayı VERMEZ; 22 Temmuz 2026
   itibarıyla kurucu bu onayı henüz VERMEDİ).
2. `ledger_outbox_ayarlari`, `ledger_outbox_manual_retry`, `ledger_outbox_
   ...
   burada da geçerli).
3. ~~K2'nin GERÇEK açık kararı (`ADR-dis-cron.md`, dış tetik A/B/C)~~
   **VERİLDİ (§14):** şimdilik C, ilk pilot öncesi/onboarding sırasında
   A'ya geçiş (12 zorunlu kabul kriteriyle), B reddedildi. A'ya geçişin
   kendisi AYRI bir uygulama turudur ve §14'teki kriterler karşılanmadan
   başlamaz.
