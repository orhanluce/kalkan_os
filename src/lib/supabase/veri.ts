...
export async function fetchKutuphane(db: Db): Promise<Kutuphane> {
  const [fw, ctrl, map] = await Promise.all([
    ...
    db
      .from("controls")
      .select("id, framework_id, madde_ref, baslik, aciklama, kanit_tipi, periyot, kritiklik"),
...
export async function fetchKurum(db: Db): Promise<Kurum> {
  const [t, p, o] = await Promise.all([
    ...
    db
      ...
      .select(
        "organization_type, regulated_entity_types, finance_department_enabled, profil_tamamlandi_at",
      )
      .maybeSingle(),
  ]);
  ...
  return {
    ...
    organizasyon: o.data
      ? {
          ...
          regulatedEntityTypes: o.data.regulated_entity_types,
...
export async function fetchStoreState(db: Db): Promise<StoreState> {
  const [tc, ev, fnd, audit, share] = await Promise.all([
    db
      ...
      .select("id, tenant_id, control_id, durum, sorumlu_user_id, son_degerlendirme, not_metni"),
    db
      .from("evidences")
      .select(
    ...
    db
      .from("findings")
      .select(
        "id, tenant_id, kaynak, onem, baslik, aksiyon_plani, yk_onay_tarihi, hedef_kapama, durum",
      ),
    db
      .from("audit_log")
      .select("id, tenant_id, actor_id, eylem, hedef_tablo, hedef_id, detay, created_at")
      .order("seq", { ascending: false }),
    db
      .from("share_links")
      .select("id, tenant_id, token, kapsam, olusturan, son_gecerlilik, created_at"),
  ]);
  ...
  return {
    tenantControls: (tc.data ?? []).map((r): TenantControl => ({
      id: r.id,
      tenantId: r.tenant_id,
      controlId: r.control_id,
    ...
    findings: (fnd.data ?? []).map((r): Finding => ({
      id: r.id,
      tenantId: r.tenant_id,
    ...
    auditLog: (audit.data ?? []).map((r): AuditLogEntry => ({
      id: r.id,
      tenantId: r.tenant_id,
      actorId: r.actor_id,
