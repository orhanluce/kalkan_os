-- 37 Tez Dikey B, Faz 4 (ADR §5): Proof Room roi_export dalına ledgerDurumu +
-- MİNİMİZE EDİLMİŞ alan bazlı provenance özeti eklendi.
--
-- FORWARD-FIX DİSİPLİNİ (dünkü dersin bir kez daha uygulanması, kural: yalnız
-- ilk migration değil GÜNCEL hali temel al): grep -rl "proof_room_goruntule"
-- supabase/migrations/ → en güncel sürüm 20260720160000 (ledgerDurumu'nun
-- test_runs dalına geri getirildiği forward-fix). Bu migration O sürümü
-- TAMAMINI temel alıyor, yalnız roiExport dalına iki alan ekliyor.
--
-- MİNİMİZASYON (ham hassas veri Proof Room'a taşınmaz): provenance_raporu
-- ham HALİYLE dönmez — yalnız {alanKodu, kaynakDurumu, genelDurum,
-- iddiaSayisi} projeksiyonu (satirId/iddia detayı/kaynak metni YOK).
