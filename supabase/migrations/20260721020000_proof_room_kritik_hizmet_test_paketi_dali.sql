-- Dikey F, F2: proof_room_goruntule() BEŞİNCİ dal — kritik_hizmet_test_
-- paketi_snapshot_id. GÜNCEL sürüm (20260720330000, grep doğrulandı — en son
-- forward-fix) TAM olarak temel alındı; yalnız yeni bir if bloğu EKLENDİ,
-- mevcut dört dal DEĞİŞMEDİ.
--
-- Diğer üç snapshot dalıyla (cloud_assurance/graph_snapshot) AYNI desen:
-- mühürlenmiş `paket` JSONB'si OLDUĞU GİBİ döner — motor (src/lib/kritik-
-- hizmet-test-paketi.ts) zaten ham log/uzun gözlem/kanıt dosyası içeriği/PII
-- taşımayacak şekilde tasarlandı (ADR §2.4/§3), bu yüzden okuma anında ayrıca
-- filtrelemeye gerek yok — mühürlenen şey zaten minimize.
