-- Dikey F, F4: proof_room_goruntule() test_run_id dalı, GÜNCEL kurtarma
-- ölçümünü İLİŞKİSEL olarak ekler. ALTINCI polimorfik hedef AÇILMAZ (F3 dersi)
-- — ölçüm bir koşuya bağlı, zaten var olan test_run_id dalından minimize özet
-- olarak sorgulanır. GÜNCEL sürüm 20260721020000 (F2 beşinci dal) TAM temel
-- alındı; diğer dallar ve test_run_id dalının mevcut alanları DEĞİŞMEDİ,
-- yalnız 'kurtarmaOlcumu' anahtarı EKLENDİ.
--
-- MİNİMİZASYON: ham beyan_eden UUID'si DÖNMEZ (declarantPresent boolean);
-- yalnız kaynak/mod/süreler/birim + "beyandır" bağlamı. "Güncel" = kimse
-- supersede etmemiş kayıt (türetilir). Nicel karşılaştırma YOK.
...
begin
  ...
  -- --- Dikey F, F4: GÜNCEL kurtarma ölçümü (minimize; ham beyan_eden DÖNMEZ) ---
  -- "Güncel" = kimse supersede etmemiş kayıt. Nicel karşılaştırma YOK.
  select jsonb_build_object(
      'olcumKaynagi', m.olcum_kaynagi,
      'girdiModu', m.girdi_modu,
      'olculenKesintiSaat', m.olculen_kesinti_saat,
      'olculenVeriKaybiSaat', m.olculen_veri_kaybi_saat,
      'beyanKesintiSaat', m.beyan_kesinti_saat,
      'beyanVeriKaybiSaat', m.beyan_veri_kaybi_saat,
      'birim', 'SAAT',
      'olcumHash', m.olcum_hash,
      'olcumZamani', m.measured_at,
      'karsilastirmaYapildi', false
    ) into v_kurtarma_olcumu
  from public.test_run_recovery_measurements m
  where m.test_run_id = v_run.id
    and m.tenant_id = v_link.tenant_id
    and not exists (
      select 1 from public.test_run_recovery_measurements m2 where m2.supersedes_measurement_id = m.id
    )
